...
class UserViewSet(mixins.ListModelMixin,mixins.UpdateModelMixin,GenericViewSet):
    ...
    def list_users(self, request, *args, **kwargs):
        org_id = request.user.org_id
        self.queryset = UserInfo.objects.filter(org=org_id).exclude(role_priv='org_admin').exclude(role_priv='super_admin').exclude(id=request.user.id)
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({'error':"this role does not exist"},status=status.HTTP_400_BAD_REQUEST)
